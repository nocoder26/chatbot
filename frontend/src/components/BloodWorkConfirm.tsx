...
interface BloodWorkConfirmProps {
  ...
  onConfirm: (data: { results: LabResult[] }) => void;
  onCancel: () => void;
}
...
export default function BloodWorkConfirm({
  ...
  const missingMarkers = useMemo(() => {
    ...
    return FERTILITY_MARKERS.filter(
      (m) => !existingNames.some((name) => name.includes(m.name.toUpperCase()))
    );
  }, [results]);

  const handleRowChange = (index: number, field: keyof LabResult, value: string) => {
  ...
  return (
    <div className="p-6 bg-[#f9f9f9] dark:bg-[#1a1a1a] shadow-2xl rounded-[2rem] border border-black/5 dark:border-white/5 max-w-2xl w-full max-h-[90vh] flex flex-col relative overflow-hidden">
      ...
      <div className="shrink-0 mb-6 pr-8">
        <button
          onClick={onCancel}
          className="absolute right-6 top-6 p-2 rounded-full hover:bg-black/5 dark:hover:bg-white/5 transition-colors"
        >
          <X className="w-5 h-5 text-slate-400" />
        </button>
        <h2 className="text-2xl font-bold text-[#3231b1] dark:text-[#86eae9] tracking-tight">
          Review Lab Results
        </h2>
          ...
          {results.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {results.map((row, i) => (
                <div
                  key={i}
                  className="group relative bg-white dark:bg-white/5 p-4 rounded-2xl border border-slate-200 dark:border-white/10 hover:border-[#3231b1] dark:hover:border-[#86eae9] transition-all shadow-sm"
                >
